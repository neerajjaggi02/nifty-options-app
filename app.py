...
def apply_sl_target(df):
    ...
    # Initialize StopLoss and Target columns with NaN values
    df["StopLoss"] = pd.NA
    df["Target"] = pd.NA

    # Iterate efficiently using vectorized operations
    buy_entries = df["Close"] * (1 - STOP_LOSS_PCT)
    sell_entries = df["Close"] * (1 + STOP_LOSS_PCT)

    target_buy = df["Close"] * (1 + TARGET_PCT)
    target_sell = df["Close"] * (1 - TARGET_PCT)

    df.loc[df["Signal"] == "Buy", "StopLoss"] = buy_entries
    df.loc[df["Signal"] == "Buy", "Target"] = target_buy

    df.loc[df["Signal"] == "Sell", "StopLoss"] = sell_entries
    df.loc[df["Signal"] == "Sell", "Target"] = target_sell
